...
## Task 0: Fix the undeclared `tzdata` dependency

**Why:** `cinepipeline/core/models.py` calls `ZoneInfo("Europe/Paris")` at import time.
On Linux this reads the system tz database. **Windows has no system tz database**, so it
needs the `tzdata` PyPI package. It is currently installed in `.venv` by accident and is
not declared, so a fresh `pip install -e .` on Windows produces a crash on import.
...
If it refuses because `web/` is not empty, create the files by hand as specified below —
do **not** delete `web/public/`.
